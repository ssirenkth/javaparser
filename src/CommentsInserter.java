...
import static com.github.javaparser.StaticJavaParser.testArray;
...
class CommentsInserter {
     ...
     */
    void insertComments(Node node, TreeSet<Comment> commentsToAttribute) { //+1 #11
        testArray[11] = 1;
        if (commentsToAttribute.isEmpty()){// +1 #12
            testArray[12] = 1;
            return;
        }


        if (node instanceof CompilationUnit) {//+1 #13
            testArray[13] = 1;
        ...
        for (Node thing : childrenAndComments) { //+1 #15
            testArray[15] = 1;
            if (thing instanceof Comment) { //+1 #16
                testArray[16] = 1;
                previousComment = (Comment) thing;
                if (!previousComment.isOrphan()) {//+1 #17
                    testArray[17] = 1;
                    previousComment = null;
                }
            } else {
                if (previousComment != null && !thing.getComment().isPresent()) {//+2 #18
                    testArray[18] = 1;
                    if (!configuration.isDoNotAssignCommentsPrecedingEmptyLines()//+1 #19
                            || !thereAreLinesBetween(previousComment, thing)) { //+1  #19
                        testArray[19] = 1;
        ...
        for (Comment c : commentsToAttribute) {//+1 #20
            testArray[20] = 1;
            if (c.isOrphan()) {//+1 #21
                testArray[21] = 1;
